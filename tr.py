...
def create_association_matrix(tokens, No_of_unique_chars):
    association_matrix = np.zeros(
        shape=(No_of_unique_chars, No_of_unique_chars))
...
def calculate_word_ranks(association_matrix, counter_vector, max_iterations=50, min_iterations=10, threshold=0.000001, damping_factor=0.85):
    ...
    old_importance_vector = np.full(
        shape=no_of_unique_word, fill_value=1 / no_of_unique_word)
    for itt in range(max_iterations):
        for i in range(no_of_unique_word):
            ...
            new_importance_vector[i] = (
                1-damping_factor) + damping_factor*(sum)
        new_importance_vector = normalize_importance_vector(
            new_importance_vector)
